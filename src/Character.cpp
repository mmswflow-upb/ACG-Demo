...
#include "../includes/ProjectileType.h" // For getProjectileAttributes
#include <iostream>
...
Character::Character(
    ...
    float lookAngle,
    bool walking
) : characterType(characterType), position(position), scaling(scaling), lookAngle(lookAngle), walking(walking), lastShotTime(0.0f) , lastWalkingAnimationTime(0.0f){
...
//renders character
void Character::render(GLuint programID, float deltaTime) {


    GLuint currentTextureID;
    static int textureIndex = 0; // Persist across calls

    lastWalkingAnimationTime += deltaTime;

    // Alternate between walking textures if moving every ~0.2 seconds
    if (walking && !walkingTextureIDs.empty()) {
        if (lastWalkingAnimationTime >= 0.2f) {

            // Determine the next texture index
            textureIndex = (textureIndex + 1) % walkingTextureIDs.size();

            // Reset the timer
            lastWalkingAnimationTime = 0.0f;
        }
        //Update currentTextureID based on how much time has elapsed since the last transition (when walking)
...
// Move enemies' characters between -0.8 and 0.8 on x or y axis
...
//Shooting function (initializes new projectiles)
...
//Setting up character's rectangle on which the texture is placed
