...
Projectile::Projectile(ProjectileType type, glm::vec3 position, glm::vec3 target)
    ...
    setupProjectile();
    //Compute direction based on received target & compute angle to make it face the target
...
void Projectile::render(GLuint projectilesProgramID) {
    ...
    transform = glm::scale(transform, glm::vec3(1.0f, 0.08f, 1.0f)); //Scale projectile to make it look slim
...
bool Projectile::updatePosition(float deltaTime) {
    ...
    position += direction * speed * deltaTime; //Move the projectile in the originally computed direction
